...
void	ft_fractal_newton(t_param *p, double x, double y)
{
	...
	//z = ft_make_cplx(((x/* + p->ox*/) * p->zoom[p->fractal]) / WIDTH - p->zoom[p->fractal] * 0.5, 		\
	//((y/* + p->oy*/) * p->zoom[p->fractal]) / HEIGHT - p->zoom[p->fractal] * 0.5);
	i = -1;
	while (!(nb = ft_check_epsilon(z, p->deg_newton[p->fractal - 18], 0.00001)) 				\
			&& ++i < p->it[p->fractal] * p->deg_newton[p->fractal - 18])
	...
	if (p->deg_newton[p->fractal - 18] <= 7)
	{
		...
		ft_pixel_put(p->mlx->win[p->fractal], x, y, 						\
				...
				BLACK, i / (p->it[p->fractal] * p->deg_newton[p->fractal - 18])));
	}
	else if (nb)
		ft_pixel_put(p->mlx->win[p->fractal], x, y, 						\
