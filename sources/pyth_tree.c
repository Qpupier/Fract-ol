...
/*   Updated: 2019/06/12 19:46:55 by qpupier     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
	...
	while (q.side ? (--i > xy.x - q.size) : (++i < xy.x + q.size))
	{
		...
		while (--j > xy.y - q.size)
		{
			...
			ft_pixel_put(p->mlx->win[p->fractal], 				\
					c.rl + xy.x, c.im + xy.y, q.nb < p->it[20] / 4.0 ? BROWN : ft_color_average(GREEN, ft_color_hex_to_rgb("0x228B22"), (double)q.nb / (p->it[p->fractal] + p->it_tmp[p->fractal] - 1)));
		}
	}
...
int		ft_begin_pyth(t_param *p, int y, int size)
{
	int	i;
	int	j;
	...
	while (++i < 400 / p->zoom[p->fractal])
	{
		...
		while (++j < 400 / p->zoom[p->fractal])
			ft_pixel_put(p->mlx->win[p->fractal], 							\
					ft_inverse_coords(p, 0, 0).rl - 400 / p->zoom[p->fractal] * 0.5 + j, 	\
					ft_inverse_coords(p, 0, 2).im - 1200 / p->zoom[p->fractal] + i, BROWN);
	}
	y += size;
	return (y);
}
...
void	ft_tree(t_param *p, t_cplx xy, double tree_angle, t_tree t)
{
	...
	if (!t.nb)
	{
		ft_line(img, xy, ft_cplx_add(xy, ft_make_cplx(0, -t.size * 2)), p->colors_tree[0]);
	...
	ft_line(img, xy, l, ft_color_average(p->colors_tree[0], p->colors_tree[1], (double)t.nb / p->it[24]));
	ft_line(img, xy, r, ft_color_average(p->colors_tree[0], p->colors_tree[1], (double)t.nb / p->it[24]));
	ft_tree(p, l, tree_angle + ft_deg_to_rad(p->tree_degree[0]), (t_tree){t.size * p->tree_ratio, t.nb + 1});
	ft_tree(p, r, tree_angle + ft_deg_to_rad(p->tree_degree[2]), (t_tree){t.size * p->tree_ratio, t.nb + 1});
	if (p->tree_nb == 3)
	{
		m = ft_cplx_turn(ft_cplx_remove(xy, ft_make_cplx(0, t.size)), xy, 	\
				tree_angle + ft_deg_to_rad(p->tree_degree[1]));
		ft_line(img, xy, m, ft_color_average(p->colors_tree[0], p->colors_tree[1], (double)t.nb / p->it[24]));
		ft_tree(p, m, tree_angle + ft_deg_to_rad(p->tree_degree[1]), (t_tree){t.size * p->tree_ratio, t.nb + 1});
	}
}
