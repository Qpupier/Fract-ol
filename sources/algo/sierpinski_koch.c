...
/*   Updated: 2019/06/19 15:15:45 by qpupier     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
void		ft_sierpinski_carpet(t_param *p, t_real o, t_real coord, int nb)
{
	...
	if (nb > p->it[21] || nb <= 0)
		return ;
	color = ft_sierpinski_color(p, nb);
	j = o.y + coord.y / 3 - 1;
...
static void	ft_draw_triangle(t_param *p, t_real o, int size, t_cplx pt)
{
	int	i;
...
static void	ft_koch(t_param *p, t_cplx d, t_cplx e, int nb)
{
	t_cplx	nb1;
	t_cplx	nb2;
	t_cplx	nb3;
	...
	nb1 = ft_cplx_average(d, e, (double)1 / 3);
	nb2 = ft_cplx_average(d, e, (double)2 / 3);
...
void		ft_size_koch(t_param *p, t_cplx o, int w, int h)
{
	...
	if (p->vignette)
		while (++i < DIS / 2)
		{
			j = HEIGHT - HEIGHT / 5 - 1;
			while (++j < HEIGHT)
				ft_pixel_put(p->mlx->param, i, j, BLACK);
		}
