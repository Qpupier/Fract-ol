...
static t_cplx	ft_result_newton(t_param *p, t_cplx z, int deg)
{
	int	a;
	int	b;
...
static int		ft_check_epsilon(t_cplx z, int deg, double e)
{
	int	i;
...
void			ft_fractal_newton(t_param *p, double x, double y)
{
	...
	i = -1;
	while (!(nb = ft_check_epsilon(z, p->deg_newton[p->fractal - 18], 		\
			0.00001)) && ++i < (p->it[p->fractal] + p->it_tmp[p->fractal]) 	\
	...
	if (p->deg_newton[p->fractal - 18] <= 7)
		ft_pixel_put(p->mlx->win[p->fractal], x, y, 						\
				ft_color_average(ft_color_int_to_rgb(--nb < 3 				\
				? RED_H >> nb * 8 : 										\
				~((RED_H >> (nb - 3) * 8) | 0xFF000000)), BLACK, 			\
				i / ((p->it[p->fractal] + p->it_tmp[p->fractal]) * 			\
				p->deg_newton[p->fractal - 18])));
	else if (nb)
		ft_pixel_put(p->mlx->win[p->fractal], x, y, 						\
